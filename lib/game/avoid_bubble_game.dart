...
class AvoidBubbleGame extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    // 오디오 서비스 초기화
    AudioService.instance.updateSettings(settings);
    ...
    // 개발자 모드에서 히트박스 토글 안내 표시
    if (GameSettings.isDeveloperModeAvailable) {
      final hitboxHintText = TextComponent(
        text: 'Press H to toggle hitboxes',
  ...
  void spawnEightDirectionBullets() {
    ...
    final safeDistance = size.length / 2 + 100;
  ...
  void resetGame() {
    ...
    try {
      ...
      // 플레이어나 timeText가 아직 초기화되지 않은 경우 무시
    }
  }
  ...
  // 히트박스 표시 오버라이드 플래그
  bool? _showHitboxesOverride;

  // 실제 히트박스 표시 여부 getter (오버라이드 고려)
  bool get shouldShowHitboxes => _showHitboxesOverride ?? settings.showHitboxes;
